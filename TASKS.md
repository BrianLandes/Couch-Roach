...
- Tests: resolver tier reorder (pack-first / single-episode fallback), `SeasonPackSourceRepository` (remember/replace/independent keys/forget). 514 green, analyze clean. Device-verify: play an episode of a show with a season pack (should grab the pack), then play another episode of the same season (should reuse it, no re-search).
