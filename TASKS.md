...
- ⚠ **Unverified here:** the C++ can't be compiled in the Linux CI container — needs the **Windows CI build to succeed** and an **on-device test** (key pauses Couch Roach only; Spotify no longer starts; no double-toggle). Dart analyze + 528 tests green. If the Windows build breaks or a double-toggle shows up, that's the follow-up.
